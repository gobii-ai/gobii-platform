...
class AgentChatConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def usage_update_event(self, event):
        await self.send_json({"type": "usage.updated", "payload": event.get("payload")})
...
class AgentChatSessionConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def usage_update_event(self, event):
        await self._send_agent_event("usage.updated", event)
