...
def _pipedream_owner_for_agent(agent: PersistentAgent) -> tuple[str, str, Any | None, Any | None, str]:
    if agent.organization_id:
...
def _serialize_agent_pipedream_app_row(
    app: dict[str, str],
    *,
    source: str,
...
class AgentPipedreamAppsAPIView(ApiLoginRequiredMixin, View):
    ...
    def get(self, request: HttpRequest, agent_id: str, *args: Any, **kwargs: Any):
        ...
        owner_scope, owner_label, owner_user, owner_org, _owner_id = _pipedream_owner_for_agent(agent)
        state = get_owner_apps_state(owner_scope, owner_label, owner_user=owner_user, owner_org=owner_org)
        ...
        try:
            if query:
                search_results = catalog.search_apps(query, limit=30)
        ...
        for slug in ordered_slugs:
            ...
            if slug in platform_set:
                ...
                source = "available"
            rows.append(
                _serialize_agent_pipedream_app_row(
                    app,
                    source=source,
...
class AgentPipedreamAppAPIView(ApiLoginRequiredMixin, View):
    ...
    def delete(self, request: HttpRequest, agent_id: str, app_slug: str, *args: Any, **kwargs: Any):
        status = pipedream_status()
        if not status.enabled:
            return JsonResponse({"error": status.reason}, status=503)
        ...
        if not normalized_slug:
            return HttpResponseBadRequest("app_slug is required.")
        ...
        owner_scope, owner_label, owner_user, owner_org, owner_id = _pipedream_owner_for_agent(agent)
        state = get_owner_apps_state(owner_scope, owner_label, owner_user=owner_user, owner_org=owner_org)
        if normalized_slug in set(state.platform_app_slugs):
            return JsonResponse({"error": "Built-in apps cannot be removed."}, status=400)
...
class AgentPipedreamAppConnectAPIView(ApiLoginRequiredMixin, View):
    ...
    def post(self, request: HttpRequest, agent_id: str, app_slug: str, *args: Any, **kwargs: Any):
        status = pipedream_status()
        if not status.enabled:
            return JsonResponse({"error": status.reason}, status=503)
        ...
        connect_url = request.build_absolute_uri(
            reverse(
                ...
                kwargs={"agent_id": agent.id, "app_slug": normalized_slug},
            )
        )
        return JsonResponse(
            {
...
class PipedreamAppAgentConnectionsAPIView(ApiLoginRequiredMixin, View):
    ...
    def get(self, request: HttpRequest, app_slug: str, *args: Any, **kwargs: Any):
        status = pipedream_status()
        if not status.enabled:
            return JsonResponse({"error": status.reason}, status=503)
        ...
        return JsonResponse({"app_slug": normalized_slug, "agents": rows})
