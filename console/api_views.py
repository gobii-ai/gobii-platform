...
from console.forms import AgentEmailAccountConsoleForm, MCPServerConfigForm, PhoneAddForm, PhoneVerifyForm
...
def _copy_agent_email_account_data(source: AgentEmailAccount, target: AgentEmailAccount) -> None:
    ...
    target.smtp_password_encrypted = source.smtp_password_encrypted
    target.imap_password_encrypted = source.imap_password_encrypted
...
def _ensure_agent_email_endpoint_and_account(
    ...
    with transaction.atomic():
        ...
        if existing_endpoint and existing_endpoint.owner_agent_id and existing_endpoint.owner_agent_id != agent.id:
            raise ValidationError({"endpoint_address": "That email address is already assigned to another agent."})
        ...
        if not endpoint:
            if existing_endpoint:
                ...
                if endpoint.address != normalized:
                    endpoint.address = normalized
