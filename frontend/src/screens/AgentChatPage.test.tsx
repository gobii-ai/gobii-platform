    ...
    AgentChatLayout: ({
      ...
      return (
        <div>
          ...
          <div data-testid="telegram-native-tab-enabled">{String(Boolean(telegramNativeTabEnabled))}</div>
          <button
            type="button"
...
describe('AgentChatPage trial onboarding', () => {
  ...
  it('ignores malformed live tool search results when checking for Sheets skill enablement', async () => {
    ...
    expect(screen.getByTestId('telegram-native-tab-enabled')).toHaveTextContent('false')
  })

  it('passes native tab enablement from roster system skills', async () => {
    rosterState.agents = [buildRosterAgent('agent-1', 'Agent One', ['apollo_native', 'hubspot_native', 'discord_native', 'telegram_native'])]
    window.history.pushState({}, '', '/app/agents/agent-1')
    ...
    expect(screen.getByTestId('telegram-native-tab-enabled')).toHaveTextContent('true')
  })
        ...
        entries: [
          {
            ...
            result: {
              status: 'success',
              system_skills: {
                enabled: ['apollo_native', 'hubspot_native', 'discord_native', 'telegram_native'],
              },
            },
            status: 'complete',
          },
