      ...
      <div className={getSettingsSurfaceClassName({ variant: 'embedded', shadowClassName: 'shadow-none' })}>
        <div className="flex flex-col gap-3 px-5 py-4">
          ...
          {uploadMutation.isPending && uploadInfo ? (
            <div className="flex flex-wrap items-center gap-3 rounded-lg border border-blue-300/30 bg-blue-950/20 px-3 py-2 text-sm text-blue-100">
              <RefreshCw className="h-4 w-4 animate-spin" aria-hidden="true" />
              <span>
                Uploading {uploadInfo.fileCount} file{uploadInfo.fileCount === 1 ? '' : 's'} to {uploadTargetName}
              </span>
              <span className="h-1.5 w-32 overflow-hidden rounded-full bg-blue-900/60">
                <span className="block h-full w-1/2 animate-pulse rounded-full bg-blue-300" />
              </span>
            </div>
          ) : null}
