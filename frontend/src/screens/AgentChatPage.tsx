...
const TELEGRAM_NATIVE_SYSTEM_SKILL_KEY = 'telegram_native'
...
export function AgentChatPage({
  ...
  const rosterTelegramNativeTabEnabled = Boolean(
    activeRosterMeta?.enabledSystemSkills?.includes(TELEGRAM_NATIVE_SYSTEM_SKILL_KEY),
  )
  ...
  const liveTelegramNativeTabEnabled = useMemo(
    () => Boolean(activeAgentId && timelineHasSystemSkillEnablement(timelineEvents, TELEGRAM_NATIVE_SYSTEM_SKILL_KEY)),
    [activeAgentId, timelineEvents],
  )
  ...
  const telegramNativeTabEnabled = rosterTelegramNativeTabEnabled || liveTelegramNativeTabEnabled
  useEffect(() => {
    if (!activeAgentId || (!liveGoogleSheetsDriveTabEnabled && !liveApolloNativeTabEnabled && !liveHubSpotNativeTabEnabled && !liveDiscordNativeTabEnabled && !liveTelegramNativeTabEnabled)) {
      return
    }
    const refreshKey = [
      activeAgentId,
      ...
      liveTelegramNativeTabEnabled ? TELEGRAM_NATIVE_SYSTEM_SKILL_KEY : '',
    ].join(':')
    ...
    void queryClient.invalidateQueries({ queryKey: ['agent-roster'] })
  }, [activeAgentId, liveApolloNativeTabEnabled, liveDiscordNativeTabEnabled, liveGoogleSheetsDriveTabEnabled, liveHubSpotNativeTabEnabled, liveTelegramNativeTabEnabled, queryClient])
