...
function buildToolEntryDisplay(
  ...
  return {
    ...
    iconSrc: mergedTransform.iconSrc ?? null,
...
function buildToolEntries(clusterCursor: string, entry: ToolCallEntry, options: ToolDisplayOptions = {}): ToolEntryDisplay[] {
  ...
  if (entry.developerEvent) {
    ...
    return [{
      ...
      iconSrc: null,
      iconBgClass: completion ? 'bg-violet-100' : step?.is_system ? 'bg-amber-100' : 'bg-indigo-100',
      iconColorClass: completion ? 'text-violet-700' : step?.is_system ? 'text-amber-700' : 'text-indigo-700',
      parameters,
      rawParameters: entry.parameters,
      result: entry.result,
      summary: null,
