...
function isBrandedApiEntry(entry: ToolEntryDisplay): boolean {
  return Boolean(entry.iconSrc && BRANDED_API_ICON_SRCS.has(entry.iconSrc))
}
...
function deriveEntryVisual(entry: ToolEntryDisplay, activity: ActivityDescriptor): EntryVisual {
  ...
  const scrapeTargets = activity.kind === 'linkedin' || isBrandedApiEntry(entry)
    ? []
...
function classifyActivity(entry: ToolEntryDisplay): ActivityKind {
  ...
  if (isBrandedApiEntry(entry)) return 'tool'
