...
function extractSqliteTargetLabel(
  ...
  switch (kind) {
    ...
    case 'agentSkills':
      return (
        extractFieldFromSql(statement, ['name']) ??
        extractFieldFromResult(result, ['name'])
      )
    case 'files':
      return (
...
function extractSqliteResultStatus(result: unknown): string | null {
  const resultObject = parseResultObject(result)
...
export function getSqliteInternalTableDisplay(
  ...
  let caption = targetLabel ? truncate(targetLabel, 56) : descriptor.labelPrefix
  let summary = `Interacting with ${descriptor.tableName}.`

  if (kind === 'messages') {
    summary = summaryKind === 'query' ? 'Querying the agent message snapshot.' : 'Modifying the agent message snapshot.'
  } else if (kind === 'toolResults') {
    summary = summaryKind === 'query' ? 'Querying stored tool output snapshots.' : 'Modifying stored tool output snapshots.'
    ...
    summary = summaryKind === 'query' ? 'Querying the agent file index snapshot.' : 'Modifying the agent file index snapshot.'
  }
