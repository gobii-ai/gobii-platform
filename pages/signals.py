...
def handle_invoice_payment_failed(event, **kwargs):
    ...
    with tracer.start_as_current_span("handle_invoice_payment_failed") as span:
        ...
        total_amount = payload.get("amount_due")
        if total_amount is None:
            total_amount = payload.get("total")
        amount_major = None
        try:
            amount_major = float(Decimal(str(total_amount)) / Decimal("100")) if total_amount is not None else None
        except (InvalidOperation, TypeError, ValueError):
            amount_major = None
