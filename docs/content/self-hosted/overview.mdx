...
Bright Data web search and markdown scraping use the native Bright Data API. Other Bright Data tools continue to use the preconfigured platform MCP server.

1. Set both `BRIGHT_DATA_TOKEN` and `BRIGHT_DATA_WEB_UNLOCKER_ZONE` in your deployment environment.
   - For Docker Compose, place them in a `.env` file and pass them into the `web` and `worker` services (or use an `env_file`).
   - The zone must already exist; Gobii does not create or modify Bright Data zones.
   - Optionally set `BRIGHT_DATA_WEB_UNLOCKER_ZONE_FALLBACK` to retry failed native search and markdown scraping requests once with a second existing zone.
2. Restart the stack so the containers pick up the variables.
3. Native Bright Data search and markdown scraping are enabled by default. In the Console, use **MCP Servers** to manage any additional Bright Data tools.
