...
| `gobii_list_agent_contact_endpoints` | List agent-owned and preferred contact endpoints. |
| `gobii_set_agent_preferred_contact_endpoint` | Set or clear the preferred contact endpoint using an endpoint id or active contact id. |
...
`gobii_set_agent_preferred_contact_endpoint` accepts either an accessible endpoint id or an active allowlist contact id. When passed a contact id, Gobii resolves or creates the matching endpoint. Endpoint ids must be safe for the agent: agent-owned endpoints, the current preferred endpoint, or unowned endpoints for active manual allowlist contacts.
