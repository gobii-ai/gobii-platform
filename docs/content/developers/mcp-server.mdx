...
| `gobii_list_agent_contacts` | List manual allowlist contacts for an agent. |
| `gobii_add_agent_contact` | Add, reactivate, or update an email/SMS allowlist contact. |
| `gobii_remove_agent_contact` | Remove an allowlist contact by contact id or safe channel/address match. |
| `gobii_list_pending_agent_contacts` | List pending contact approval requests created by an agent. |
| `gobii_approve_pending_agent_contact` | Approve a pending contact request and add or reactivate the allowlist contact. |
| `gobii_list_agent_contact_endpoints` | List agent-owned endpoints, owner/user contact routes, and the current preferred contact endpoint. |
| `gobii_set_agent_preferred_contact_endpoint` | Set or clear the preferred contact endpoint using an owner/user contact route endpoint id or matching active contact id. |
...
Gobii contact MCP tools use the same production models as the console:

- Active/manual contacts are `CommsAllowlistEntry` rows on the agent.
- Pending approvals are `CommsAllowlistRequest` rows created when an agent asks the owner to approve a new contact.
- Contact endpoints are `PersistentAgentCommsEndpoint` rows. They are distinct from allowlist contacts; the preferred contact endpoint is stored on the agent.

`gobii_add_agent_contact` supports email and SMS allowlist contacts. It avoids duplicates by updating or reactivating an existing allowlist row for the same agent, channel, and address. `gobii_remove_agent_contact` deletes the manual allowlist row and clears `preferred_contact_endpoint` when the removed contact was the preferred endpoint.

`gobii_approve_pending_agent_contact` approves a pending contact request directly into the manual allowlist. It does not create a separate invitation in the MCP flow. Expired or already-responded requests return an MCP tool error with structured details.

`gobii_set_agent_preferred_contact_endpoint` sets the endpoint Gobii uses to reach the agent owner/user by default. MCP only accepts unowned owner/user contact routes, such as the personal owner's email/SMS/web route or an owner/admin/billing user route for organization agents. Agent-owned sender endpoints and arbitrary manual allowlist contacts are rejected. A current preferred endpoint may still be listed for compatibility and can always be cleared with `clear: true`; `can_be_preferred` shows whether an endpoint is eligible for selection.

Active allowlist entries do not currently store labels or display names, so contact list results return `label` and `name` as `null`. Pending contact requests may include a requested `name`, `reason`, and `purpose`.
...
5. `gobii_add_agent_contact`
6. `gobii_list_agent_contacts`
7. `gobii_set_agent_preferred_contact_endpoint`
8. `gobii_list_agent_contact_endpoints`
9. `gobii_remove_agent_contact`
10. `gobii_link_agents`
11. `gobii_send_agent_message`
12. `gobii_get_agent_timeline` and capture `latest_cursor`
13. `gobii_wait_for_agent_event` with a cursor and a short timeout; verify both match and timeout behavior as appropriate
14. `gobii_upload_agent_file`
15. `gobii_list_agent_files`
16. `gobii_unlink_agents`
17. `gobii_archive_agent` for temporary agents
