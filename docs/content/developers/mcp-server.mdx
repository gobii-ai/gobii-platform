...
After an agent has received an MCP message within the configured web-session retention window, it can use `send_mcp_message` from any run. The tool appends its reply to the newest qualifying MCP conversation. The outbound event uses channel `mcp`, and its `inReplyToMessageId` points to that inbound message so clients can correlate the response without treating the underlying web conversation as a human delivery channel.
