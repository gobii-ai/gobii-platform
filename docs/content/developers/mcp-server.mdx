...
`gobii_send_agent_message` appends an authenticated MCP-originated message to the agent's unified timeline. The event uses channel `mcp`, source kind `mcp`, and the stable source label `Gobii MCP`; it does not impersonate the API key's user as the visible sender. The tool returns `message_id`, `agent_id`, `cursor`, `latest_cursor`, `created_at`, `accepted_state`, external actor metadata, and the serialized timeline event.
...
After an agent has received an MCP message within the configured web-session retention window, it can use `send_mcp_message` from any run. While processing an MCP request, the tool replies to that bound request; otherwise it appends to the newest qualifying MCP conversation. The outbound event uses channel `mcp`, and its `inReplyToMessageId` points to that inbound message so clients can correlate the response without treating the underlying web conversation as a human delivery channel.
...
- `from_actor_type`: `agent`, `human_user`, `external`, or `system`, derived from the serialized event. Outbound or peer messages are `agent`; inbound web user messages are `human_user`; MCP and other inbound messages are `external`; steps/thinking/plan events are `system`.
