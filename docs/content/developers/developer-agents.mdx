...
For a developer-focused product overview and verified quickstart, see the [Gobii Agent API](https://gobii.ai/solutions/engineering/) page.

Agents can use browser automation and other AI tools. They may also be configured to use MCP servers and connected app tools where available.
...
- **Receive [structured data](/developers/structured-data)** where an endpoint supports schema-guided output.
...
- `available_mcp_servers` / `personal_mcp_server_ids` (arrays): MCP server metadata and enabled personal servers where exposed.
