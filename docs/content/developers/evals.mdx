---
title: "Developer Evals"
sidebarTitle: "Evals"
description: "Run, compare, and extend Gobii's canonical eval suites from a local checkout."
---

## Why evals exist

Gobii evals are executable product checks for agent behavior. They answer questions that unit tests cannot: did the agent choose the right tool, stop at the right time, ask for approval, avoid duplicate replies, and keep behavior stable when we change prompts, tools, routing profiles, or models?

The canonical eval system lives under `api/evals` and is launched with `uv run python manage.py run_evals`. Do not add one-off management commands for feature evals. Add scenarios and suites to the canonical registry instead.

## What to run

| Run type | Purpose | Command shape | Counts as |
| --- | --- | --- | --- |
| Unit tests | Prove Python code, scoring helpers, registration, and local setup behavior. | `uv run python manage.py test ... --settings=config.test_settings` | Unit test evidence only. |
| Simulated evals | Deterministic offline eval path for scenarios that declare simulation support. | `run_evals --simulated --settings=config.eval_local_settings` | Eval runner smoke, not live model quality. |
| Live evals | Run scenarios against a real model through an LLM routing profile. | `run_evals --routing-profile <profile> --settings=config.eval_local_settings` for small slices; use `config.eval_postgres_settings` for full concurrent local suites. | Model behavior evidence. |
| Official evals | Durable comparison runs for tracked releases or model changes. | Add `--official` or `--run-type official`. | Official trend data. |

Keep those categories separate in reports. Passing unit tests does not mean evals passed, and a simulated eval does not mean a model passed.

## Architecture

| Piece | Location | Role |
| --- | --- | --- |
| `EvalScenario` | `api/evals/base.py` | Base class for one behavioral scenario. |
| `ScenarioTask` | `api/evals/base.py` | The visible assertions/tasks recorded under each run. |
| `ScenarioRegistry` | `api/evals/registry.py` | Global scenario registry. |
| `ScenarioMetadata` | `api/evals/base.py` | First-class scenario taxonomy: tags, tier, category, runtime, cost class, owner, area, simulation support, and fixtures/secrets. |
| Scenario catalog helpers | `api/evals/catalog.py` | Normalizes metadata, filters scenarios, and serializes the console catalog. |
| `SuiteRegistry` | `api/evals/suites.py` | Named groups of scenario slugs, plus the dynamic `all` suite. |
| `EvalRunner` | `api/evals/runner.py` | Executes one `EvalRun`, creates `EvalRunTask` rows, captures fingerprints and model metadata. |
| `EvalSuiteRun` | `api/models.py` | One suite invocation and its requested repeats, run type, launch config, and routing profile snapshot. |
| `EvalRun` | `api/models.py` | One scenario execution for one agent. |
| `EvalRunTask` | `api/models.py` | One scored task/assertion inside a run. |
| `run_evals` | `api/management/commands/run_evals.py` | The only CLI entry point for canonical evals. |

`api/evals/loader.py` imports scenario modules and registers built-in suites. `run_evals` loads that registry directly, so a clean CLI invocation does not depend on test imports. Specialized component scenarios may opt out of the default `core` and `all` aggregates while remaining available through their focused suite.

## Local setup

For local eval development, use `config.eval_local_settings`. It creates and migrates `.local/eval-local.sqlite3`, runs Celery work eagerly, disables browser task execution, and seeds local LLM routing profiles. It is explicit and scoped to eval-local SQLite.

For full live suites, use `config.eval_postgres_settings` with a local Postgres database. It keeps browser task execution disabled and seeds the same local routing profiles, but lets `run_evals --max-concurrency` use bounded local eager worker threads against a database that can tolerate concurrent writes.

Do not use `config.test_settings` for live evals. Test settings mock and isolate runtime behavior for unit tests; live evals need the eval-local or real deployment settings path.

Run one eval-local command at a time. The local SQLite database is intentionally simple and can lock if multiple eval-local commands try to migrate or seed profiles concurrently.

List suites and scenarios without touching the database:

```bash
uv run python manage.py run_evals --list
```

Filter the catalog by metadata:

```bash
uv run python manage.py run_evals --list --tier smoke
uv run python manage.py run_evals --list --tag meta_gobii --category meta_gobii
uv run python manage.py run_evals --list --cost-class low --runtime-class short
```

Create the local SQLite schema, seed routing profiles, and list available non-snapshot profiles:

```bash
uv run python manage.py run_evals --list-routing-profiles --settings=config.eval_local_settings
```

The eval-local profiles store env var names only. They do not store API key values.

## Common commands

Run one simulated scenario:

```bash
uv run python manage.py run_evals \
  --scenario meta_gobii_negative_content_task \
  --sync \
  --n-runs 1 \
  --simulated \
  --settings=config.eval_local_settings
```

Run a focused filtered slice:

```bash
uv run python manage.py run_evals \
  --suite meta_gobii \
  --tag contact_safety \
  --sync \
  --n-runs 1 \
  --simulated \
  --settings=config.eval_local_settings
```

Run one simulated suite:

```bash
uv run python manage.py run_evals \
  --suite meta_gobii \
  --sync \
  --n-runs 1 \
  --simulated \
  --settings=config.eval_local_settings
```

Run every registered scenario on local SQLite:

```bash
uv run python manage.py run_evals \
  --suite all \
  --sync \
  --n-runs 1 \
  --settings=config.eval_local_settings
```

For live all-suite runs, pass `--routing-profile <profile>` and expect a larger bill and a longer run. Start with one scenario or one suite before running `all`.

Run the full live suite with bounded local concurrency:

```bash
set -a; source /Users/andrew/.env-openrouter >/dev/null; set +a

uv run python manage.py run_evals \
  --suite all \
  --n-runs 1 \
  --routing-profile openrouter-deepseek-v4-flash \
  --max-concurrency 8 \
  --settings=config.eval_postgres_settings
```

Use safety caps for large live runs:

```bash
uv run python manage.py run_evals \
  --suite all \
  --n-runs 1 \
  --routing-profile openrouter-deepseek-v4-flash \
  --max-concurrency 4 \
  --max-runs-total 250 \
  --settings=config.eval_local_settings
```

`--max-concurrency` limits how many async worker runs the CLI keeps in flight while it polls results. With `--sync`, local eager execution is still one run at a time. Use `--max-runs-total` and `--max-scenarios` when a filtered command should fail closed instead of accidentally launching a broad suite.

## Live model runs

Source key files without printing values:

```bash
set -a; source /Users/andrew/.env-openrouter >/dev/null; set +a
```

OpenRouter DeepSeek V4 Flash:

```bash
uv run python manage.py run_evals \
  --suite meta_gobii \
  --sync \
  --n-runs 1 \
  --routing-profile openrouter-deepseek-v4-flash \
  --settings=config.eval_local_settings
```

Compare routing profiles in one invocation:

```bash
uv run python manage.py run_evals \
  --suite meta_gobii \
  --sync \
  --n-runs 1 \
  --routing-profile openrouter-deepseek-v4-flash \
  --routing-profile openrouter-qwen \
  --settings=config.eval_local_settings
```

Each profile gets its own `EvalSuiteRun` with a routing-profile snapshot. Use the console comparison view or compare APIs to group by model/profile after the matrix finishes.

### Compaction model comparisons

The `compaction_quality` suite directly evaluates the production communication and step
summarization prompts over 12 synthetic, production-shaped histories. Candidate profiles must
use different `summarization_endpoint` values while sharing one fixed `eval_judge_endpoint`;
the judge endpoint cannot also be a candidate's summarization endpoint.

Run the standard three-repeat comparison with one profile per candidate:

```bash
uv run python manage.py run_evals \
  --suite compaction_quality \
  --sync \
  --n-runs 3 \
  --routing-profile compaction-candidate-a \
  --routing-profile compaction-candidate-b \
  --settings=config.eval_local_settings
```

Each scenario records exactly one scored task. Opaque values such as identifiers, credentials,
URLs, and paths use verbatim checks. Human-readable facts use case- and Unicode-tolerant checks;
the semantic judge handles attribution, scope, and equivalent natural-language phrasing. A case
passes only when those required-value, superseded-value, length, and code-fence checks pass and
the fixed semantic judge passes the summary. A candidate qualifies when critical deterministic
checks pass in all 36 outcomes and at least 33 of the 36 scenario runs pass overall. Compare
qualifying models by pass rate, then candidate compaction cost, then output length. Judge calls
are not included in candidate cost.

OpenRouter Qwen:

```bash
uv run python manage.py run_evals \
  --suite meta_gobii \
  --sync \
  --n-runs 1 \
  --routing-profile openrouter-qwen \
  --settings=config.eval_local_settings
```

OpenAI:

```bash
set -a; source /path/to/your/openai.env >/dev/null; set +a

uv run python manage.py run_evals \
  --suite meta_gobii \
  --sync \
  --n-runs 1 \
  --routing-profile openai-gpt-4-1-mini \
  --settings=config.eval_local_settings
```

Custom LiteLLM model string:

```bash
set -a; source /path/to/your/provider.env >/dev/null; set +a

EVAL_LOCAL_CUSTOM_MODEL="anthropic/claude-sonnet-4-20250514" \
EVAL_LOCAL_CUSTOM_API_KEY_ENV_VAR="ANTHROPIC_API_KEY" \
uv run python manage.py run_evals \
  --suite meta_gobii \
  --sync \
  --n-runs 1 \
  --routing-profile custom-litellm \
  --settings=config.eval_local_settings
```

For OpenAI-compatible local or proxy endpoints, also set `EVAL_LOCAL_CUSTOM_API_BASE` and use the model string expected by that endpoint.

## Reading results

The CLI prints each created `EvalSuiteRun`, each scheduled `EvalRun`, task status lines, pass rate, suite status, and Developer Live Chat links for created eval agents.

The console evals page has two launch paths:

- Suite launcher: select one or more suites, choose a routing profile, set repeats, and launch.
- Scenario catalog: search scenario metadata, filter by tier/category/cost, and run one small eval quickly.

The detail page surfaces task `observed_summary`, expected summaries, token/cost rollups, Developer Live Chat links, linked artifact IDs, sanitized debug artifacts, and judge context where scenarios record it. Prefer linking or summarizing artifacts over pasting raw transcripts into issue comments.

Inspect the latest local suite run:

```bash
uv run python manage.py shell --settings=config.eval_local_settings -c "\
from api.models import EvalSuiteRun; \
r = EvalSuiteRun.objects.prefetch_related('runs__tasks').latest('created_at'); \
print(r.id, r.suite_slug, r.status, r.run_type); \
[print(run.scenario_slug, run.status, run.primary_model, run.tasks.filter(status='passed').count(), '/', run.tasks.count()) for run in r.runs.all()]"
```

Compare model experiments by running the same scenario or suite with different `--routing-profile` values. `EvalRunner` snapshots the routing profile onto the suite run and records `EvalRun.primary_model`, `scenario_fingerprint`, `code_version`, and `code_branch`.

## Scenario metadata

Every scenario inherits normalized metadata from `EvalScenario`:

| Field | Use |
| --- | --- |
| `tags` | Cross-cutting filters such as `meta_gobii`, `tool_choice`, `planning`, `contact_safety`, or `llm_judge`. |
| `tier` | Run posture: `smoke`, `core`, `extended`, or `manual`. |
| `category` | Primary behavior family, such as `conversation`, `planning`, `tool_choice`, `meta_gobii`, `global_skill`, or a tool domain. |
| `expected_runtime` | `short`, `medium`, or `long`. |
| `cost_class` | `low`, `medium`, or `high`. |
| `owner` / `area` | Who owns the eval and which product/agent area it protects. |
| `supports_simulation` | Whether `--simulated` is valid. |
| `required_fixtures` / `required_secrets` | External setup the developer must provide before the eval can run. |

Add metadata as class attributes on the scenario. Dynamic scenario factories should set metadata on the generated class so catalog filters work.

## Adding scenarios

Add new eval behavior under `api/evals/scenarios/`. A normal scenario should:

1. Subclass `EvalScenario`.
2. Define a stable `slug`, `version`, `description`, and `tasks`.
3. Define metadata: `tier`, `category`, `expected_runtime`, `cost_class`, `owner`, `area`, `tags`, `required_fixtures`, and `required_secrets` where relevant.
4. Implement `run(self, run_id: str, agent_id: str)`.
5. Use `ScenarioExecutionTools` when it needs to inject messages, trigger processing, or record task results.
6. Register itself with `ScenarioRegistry.register(...)`.
7. Ensure the module is imported by `api/evals/loader.py`.
8. Add targeted unit tests tagged with an existing eval batch tag, usually `@tag("batch_eval_fingerprint")` unless a more specific registered tag fits.

Keep scenarios deterministic where practical. If a scenario supports offline execution, set `supports_simulation = True` and make `--simulated` use deterministic local data. If it needs a real model, do not pretend the simulated path measures model quality.

## Adding suites

Suites are named groups in `api/evals/loader.py`:

```python
EvalSuite(
    slug="my_feature",
    description="Focused checks for my feature.",
    scenario_slugs=["my_feature_happy_path", "my_feature_guardrail"],
)
```

Use a focused suite when developers should run a related group repeatedly. The dynamic `all` suite covers every scenario included in the default aggregate; specialized component evals such as `compaction_quality` run only through their focused suite.

## Scoring and stop policies

Prefer small, explicit `ScenarioTask` records over one broad pass/fail. A good scenario says what behavior failed: discovery, tool choice, approval policy, output safety, stop condition, or final response quality.

For direct scoring helpers, keep pure scoring functions close to the scenario data and unit-test them. The Meta Gobii evals are the worked example: `api/evals/meta_gobii.py` defines cases and scoring, while `api/evals/scenarios/meta_gobii.py` executes simulated or live model calls through the canonical runner.

For agent-processing evals, use `api/evals/stop_policy.py` through `ScenarioExecutionTools.trigger_processing(..., eval_stop_policy=...)` when the run should stop after a terminal tool call, all expected tool calls, a tracked human-input request, or an unexpected relevant tool. Keep stop policies narrow so unrelated tool noise does not hide a regression.

When recording failures, pass useful sanitized context through `record_task_result(..., artifacts={...})`. The runner persists safe debug artifacts on `EvalRunTask.debug_artifacts` and links first message/step/browser-task records when supplied. Do not pass secret values; use secret names or status labels only.

## Meta Gobii worked example

Meta Gobii lives in the canonical suite `meta_gobii`. Its scenarios check that a manager Gobii discovers and enables the Meta Gobii system skill only when needed, plans direct Meta Gobii tools, requires confirmation for mutations, avoids legacy `spawn_agent`, handles contact output safely, and does not duplicate the same response.

Use simulated Meta Gobii runs for quick local confidence:

```bash
uv run python manage.py run_evals \
  --suite meta_gobii \
  --sync \
  --n-runs 1 \
  --simulated \
  --settings=config.eval_local_settings
```

Then run the same suite against a live routing profile before reporting model behavior:

```bash
set -a; source /Users/andrew/.env-openrouter >/dev/null; set +a

uv run python manage.py run_evals \
  --suite meta_gobii \
  --sync \
  --n-runs 1 \
  --routing-profile openrouter-deepseek-v4-flash \
  --settings=config.eval_local_settings
```

## Best practices

- Use `api/evals` and `manage.py run_evals`; do not add standalone feature-specific eval commands.
- Keep unit tests, simulated evals, live evals, and official evals separate in code reviews and final reports.
- Never hardcode provider calls inside a feature eval. Route live models through `LLMRoutingProfile`.
- Never print, commit, or store inference key values. Refer to env var names only.
- Do not use `config.test_settings` for live `run_evals`.
- Do not conflate simulated and live model results.
- Prefer one scenario or one focused suite while iterating; run `all` only when the change needs whole-registry confidence.
- Keep eval-local setup explicit. `config.eval_local_settings` is for local SQLite evals; `config.eval_postgres_settings` is for concurrent local full-suite evals. Neither is production traffic.

## Release-gate guidance

Use this as the default gate shape for prompt, tool-choice, routing, and agent-behavior changes:

| Change risk | Minimum eval evidence |
| --- | --- |
| Narrow prompt or scoring-helper edit | Relevant unit tests plus one focused simulated or live scenario. |
| Agent behavior, tool selection, or stop-policy change | Relevant unit tests plus the focused suite (`guided_planning_micro`, `tool_choice_micro`, `meta_gobii`, or another owned suite). |
| Routing profile or model change | Matrix run of the same scenario/suite across old and new profiles, compared by model/profile. |
| Broad prompt/routing/system-skill release | `--suite all --n-runs 1` live run against the release candidate profile, with failures triaged or explicitly accepted. |

Mark a run `--official` only when it is intended to be durable comparison evidence. Keep exploratory one-off runs as `one_off`.
