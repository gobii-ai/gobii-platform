...
You can start an export from either of these places:

- Open the agent and choose **Export** from its banner. On a smaller screen, look in the banner's overflow menu.
- Open **Agent Settings**, find **Export Agent** near the bottom of the page, and choose **Export Agent**.

Review the confirmation, then choose **Start Export**. Gobii prepares the ZIP in the background, so you can leave the page.

When the export is ready:

- Gobii emails you a sign-in-protected download link.
- The latest export also appears in the agent's **Export Agent** settings section.
- The download remains available for seven days.

Only the owner of a personal agent, or someone with management authority for an organization agent, can export it. Being a collaborator does not grant export access.

## Export several agents together

Use a bulk export when you are moving a personal workspace or an organization:

- In your profile, choose **Export all personal agents**.
- In an organization workspace, open the organization screen and choose **Export all organization agents**.

The ZIP contains one clearly named folder for each agent. Active and paused agents are included; deleted agents are not.
...
Return to the place where you started the export. A completed archive remains available there even if email delivery fails. Check your spam folder and confirm that your account email is verified.
