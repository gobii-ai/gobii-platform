...
$$
\text{Actual Cost} = \text{Base Task Cost} \times \text{Intelligence Multiplier}
$$
...
Every agent has a configurable daily **soft limit** for credit usage. When the agent approaches this limit, Gobii aims to stay within it. A **hard limit** (equal to the soft limit) stops the agent for the day once reached.
