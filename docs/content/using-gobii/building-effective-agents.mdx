...
Each tool you give an agent adds capability but also complexity. Only add tools the agent genuinely needs:

- **Web search / scraping** — for research agents
- **Email / SMS** — for communication agents
- **Google Sheets** — for data-tracking agents
- **MCP servers** — for API integrations (Bright Data, Pipedream, HubSpot, etc.)

Lean toward giving fewer, more focused tools. An agent with too many tools spends credits figuring out which one to use.
...
Every agent has a configurable daily soft limit. Gobii aims to stay within this budget and only slightly exceeds it when the agent is near completion of a task. A hard limit (equal to your soft limit) stops the agent entirely for the day.
...
- Check [Optimizing Credit Usage](./optimizing-credit-usage.mdx) for strategies to keep your agent team cost-efficient
