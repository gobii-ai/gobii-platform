...
Gobii lets you choose how much reasoning each agent applies. The tier you pick affects how many credits your tasks consume, so it helps to match the tier to the job.

| Tier | Best For |
|---|---|
| Standard | Simple lookups, monitoring, routine data collection |
| Premium | Research synthesis, multi-step analysis |
| Max | Complex reasoning, code review, detailed reports |
| Ultra | Deep analysis, large-context tasks |
| Ultra Max | Maximum reasoning, complex multi-step agent coordination |

Stick with the lower tiers for scheduled monitoring and simple data gathering. Reserve the top tiers for complex, one-off analysis where deep reasoning matters — they can cost many times more per task.
