...
class DatabaseModelEndpoint(models.Model):
    """Database translation endpoint configuration used by sqlite instruction translation."""

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    key = models.SlugField(max_length=96, unique=True, help_text="Endpoint key, e.g., 'openrouter_gpt41_db'")
    provider = models.ForeignKey(
        LLMProvider,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="database_endpoints",
        help_text="Optional link to the provider supplying credentials for this endpoint.",
    )
    enabled = models.BooleanField(default=True)
    low_latency = models.BooleanField(
...
class DatabaseLLMTier(models.Model):
    """Fallback tier ordering for database instruction-translation endpoints."""

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    order = models.PositiveIntegerField(unique=True, help_text="1-based order across all database tiers.")
    description = models.CharField(max_length=256, blank=True)
...
class DatabaseTierEndpoint(models.Model):
    """Weighted association between a database tier and an endpoint."""
