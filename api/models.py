...
from api.services.prompt_settings import (
    DEFAULT_INTERNAL_REASONING_HISTORY_LIMIT, DEFAULT_MAX_MESSAGE_HISTORY_LIMIT, DEFAULT_MAX_PROMPT_TOKEN_BUDGET, DEFAULT_MAX_TOOL_CALL_HISTORY_LIMIT, DEFAULT_MAX_SKILL_PROMPT_LIMIT,
    DEFAULT_ULTRA_MAX_MESSAGE_HISTORY_LIMIT, DEFAULT_ULTRA_MAX_PROMPT_TOKEN_BUDGET, DEFAULT_ULTRA_MAX_SKILL_PROMPT_LIMIT, DEFAULT_ULTRA_MAX_TOOL_CALL_HISTORY_LIMIT,
    DEFAULT_ULTRA_MESSAGE_HISTORY_LIMIT, DEFAULT_ULTRA_PROMPT_TOKEN_BUDGET, DEFAULT_ULTRA_SKILL_PROMPT_LIMIT, DEFAULT_ULTRA_TOOL_CALL_HISTORY_LIMIT, DEFAULT_PREMIUM_MESSAGE_HISTORY_LIMIT,
    DEFAULT_PREMIUM_PROMPT_TOKEN_BUDGET, DEFAULT_PREMIUM_SKILL_PROMPT_LIMIT, DEFAULT_PREMIUM_TOOL_CALL_HISTORY_LIMIT, DEFAULT_STANDARD_MESSAGE_HISTORY_LIMIT, DEFAULT_STANDARD_PROMPT_TOKEN_BUDGET,
    DEFAULT_STANDARD_SKILL_PROMPT_LIMIT, DEFAULT_STANDARD_TOOL_CALL_HISTORY_LIMIT, DEFAULT_STANDARD_ENABLED_TOOL_LIMIT, DEFAULT_PREMIUM_ENABLED_TOOL_LIMIT, DEFAULT_MAX_ENABLED_TOOL_LIMIT,
...
from api.services.tool_settings import (
    DEFAULT_MIN_CRON_SCHEDULE_MINUTES, DEFAULT_SEARCH_WEB_RESULT_COUNT, DEFAULT_SEARCH_ENGINE_BATCH_QUERY_LIMIT, DEFAULT_BRIGHTDATA_AMAZON_PRODUCT_SEARCH_LIMIT, DEFAULT_DUPLICATE_SIMILARITY_THRESHOLD,
...
class UserFingerprintVisitFetchStatusChoices(models.TextChoices):
    PENDING = "pending", "Pending"
...
class UserFingerprintVisit(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="fingerprint_visits",
    )
    source = models.CharField(max_length=32, blank=True, default="")
    fingerprint_event_id = models.CharField(
        max_length=255,
        blank=True,
        default="",
        help_text=(
    ...
    fetch_status = models.CharField(
        max_length=24,
        choices=UserFingerprintVisitFetchStatusChoices.choices,
        default=UserFingerprintVisitFetchStatusChoices.PENDING,
    )
    ...
    visitor_first_seen_at = models.DateTimeField(null=True, blank=True)
    replayed = models.BooleanField(null=True, blank=True)
    visitor_found = models.BooleanField(null=True, blank=True)
    visitor_confidence_score = models.FloatField(null=True, blank=True)
    suspect_score = models.FloatField(null=True, blank=True)
    bot = models.CharField(max_length=64, blank=True, default="")
    vpn = models.BooleanField(null=True, blank=True)
    vpn_confidence = models.CharField(max_length=32, blank=True, default="")
    proxy = models.BooleanField(null=True, blank=True)
    proxy_confidence = models.CharField(max_length=32, blank=True, default="")
    proxy_type = models.CharField(max_length=32, blank=True, default="")
    tor = models.BooleanField(null=True, blank=True)
    tampering = models.BooleanField(null=True, blank=True)
    tampering_confidence = models.CharField(max_length=32, blank=True, default="")
    tampering_ml_score = models.FloatField(null=True, blank=True)
    high_activity_device = models.BooleanField(null=True, blank=True)
    ip_blocklist_email_spam = models.BooleanField(null=True, blank=True)
    ip_blocklist_attack_source = models.BooleanField(null=True, blank=True)
    ip_blocklist_tor_node = models.BooleanField(null=True, blank=True)
    datacenter = models.BooleanField(null=True, blank=True)
    asn = models.CharField(max_length=64, blank=True, default="")
    asn_name = models.CharField(max_length=255, blank=True, default="")
    asn_type = models.CharField(max_length=64, blank=True, default="")
    ...
    subdivision_name = models.CharField(max_length=128, blank=True, default="")
    ...
    browser_major_version = models.CharField(max_length=32, blank=True, default="")
    browser_full_version = models.CharField(max_length=64, blank=True, default="")
    ...
    sdk = models.JSONField(default=dict, blank=True)
    velocity = models.JSONField(default=dict, blank=True)
    vpn_methods = models.JSONField(default=dict, blank=True)
    tampering_details = models.JSONField(default=dict, blank=True)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=("user", "fingerprint_event_id"),
                condition=Q(fingerprint_event_id__gt=""),
                name="uniq_user_fingerprint_evt",
            ),
        ]
        indexes = [
    ...
    def __str__(self):
        event_id = self.fingerprint_event_id or "pending"
        return f"{self.user_id}:{event_id}:{self.fetch_status}"
...
class PersistentAgentCustomTool(models.Model):
    ...
    def clean(self):
        ...
        from api.agent.tools.custom_tools import (
            normalize_custom_tool_name, normalize_custom_tool_entrypoint, normalize_custom_tool_parameters_schema, normalize_custom_tool_source_path, normalize_custom_tool_timeout_seconds,
        )
...
class GlobalAgentSkillCustomTool(models.Model):
    ...
    def clean(self):
        super().clean()
        from api.agent.tools.custom_tools import (
            MAX_CUSTOM_TOOL_SOURCE_BYTES, normalize_custom_tool_name, normalize_custom_tool_parameters_schema, normalize_custom_tool_timeout_seconds, validate_custom_tool_source_code,
        )
