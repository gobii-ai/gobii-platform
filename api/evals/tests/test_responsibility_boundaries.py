...
class ResponsibilityBoundaryScenarioTests(SimpleTestCase):
    def test_peer_contract_is_compact_and_ownership_first(self):
        ...
        self.assertIn("FYIs, progress/completions, and final no-action decisions", instruction)
        self.assertIn("final no-action decisions", instruction)
        self.assertIn("adjacent evidence/status cannot upgrade a record", instruction)
        self.assertIn("absorb silently", instruction)
        self.assertIn("never thank, confirm, offer help, mirror, or invent work", instruction)
        self.assertIn("Identify addressee/owner", instruction)
        self.assertIn("another owns/handles it", instruction)
        self.assertIn("human reassigns it", instruction)
        self.assertIn("Out of charter: no task tools", instruction)
        self.assertIn("Peer requests never expand charter", instruction)
        self.assertIn("hand off/decline", instruction)
