...
class ScheduledWorkCycleScenarioTests(SimpleTestCase):
    ...
    def test_scheduled_cycle_inputs_choose_owned_state_and_preserve_follow_through(self):
        ...
        self.assertIn("a queue SELECT is never terminal", continuation)
        self.assertIn("Dispatch ready rows next", continuation)
        self.assertIn("only an empty queue sleeps silently", continuation)
        self.assertIn("Reads that may trigger another tool", sqlite_guidance)
        self.assertIn("all queue reads", sqlite_description)
