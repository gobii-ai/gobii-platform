...
class FirstRunPromptCalibrationTests(TestCase):
    def test_first_run_prompt_does_not_force_progress_greeting_or_default_schedule(self):
        ...
        self.assertIn("Multi-result SQL: one query; one preview max; no snippet rows", system_prompt)
        self.assertIn("Ask/collect/confirm/scope blockers with request_human_input, not plain chat", system_prompt)
        ...
        self.assertIn("do not finalize from search results alone", system_prompt)
        self.assertIn("scrape/open at least 4 promising result URLs", system_prompt)
        self.assertIn("Search snippets are leads, not citable sources", system_prompt)
        self.assertIn("Start with one broad search", system_prompt)
        self.assertIn("instead of separate searches per company/competitor", system_prompt)
        self.assertIn("under about 5,000 chars", system_prompt)
