...
from api.agent.tags import MAX_TAGS, normalize_tags
    ...
    try:
        ...
        if isinstance(data, list):
            ...
            if isinstance(tags_field, list):
                parsed = [str(item) for item in tags_field]
            else:
                parsed = [str(value) for value in data.values()]
        else:
            parsed = [str(data)]
    except json.JSONDecodeError:
        if "," in content:
            parsed = [part for part in content.split(",")]
        else:
            parsed = [part for part in content.splitlines()]
