    ...
    return {
        ...
        "function": {
            ...
            "description": (
                ...
                "If you want `dict(row)` from SQLite query results, set `db.row_factory = sqlite3.Row` before executing the SELECT, or build dicts from `cursor.description` and tuple rows. "
                ...
                "its return value must make the side effects unambiguous. Return a dict with clear top-level fields such as status, summary, side_effects_completed, side_effects or target resource ids/names, source filters or date ranges, "
                "per-destination counts, skipped/duplicate counts when relevant, remaining_work/next_cursor for resumable work, verification advice, and a clear next_action. "
                "If the tool already wrote or synced the records, return do_not_repeat_manually=true and explain that follow-up should be read-only verification, not another append/add call. "
...
def get_custom_tools_prompt_summary(agent: PersistentAgent, *, recent_limit: int = 3) -> str:
    ...
    summary = (
        ...
        "Expose useful runtime parameters instead of hardcoding sample data, ids, filters, or destinations. "
        "For SQLite-backed transforms/syncs, include source/destination table params where appropriate plus date/status filters; for URL/list validators, accept the candidate list and destination/default options as params. "
        "Return values must be helpful to the downstream agent, especially after writes or syncs: return a dict with clear top-level fields such as status, summary, side_effects_completed, target resource ids/names or side_effects, source filters/date ranges, "
        "per-destination rows/items written, skipped/duplicate counts, remaining_work or next_cursor for resumable work, verification advice, and a concrete next_action. "
        "For completed writes, include do_not_repeat_manually=true so the agent knows to verify with read-only tools instead of manually replaying the same append/add/update calls. "
        ...
        "        'source': {'scope': 'params or SQLite filters used'},\\n"
        "        'skipped_duplicates': 0,\\n"
        "        'remaining_work': 0,\\n"
        "        'verification': 'Read results with sqlite_batch; do not repeat the write.',\\n"
        "        'next_action': 'Verify with read-only tools.',\\n"
        ...
        "cursor = conn.executemany('INSERT OR REPLACE INTO ...', rows) -> return a helpful dict with status, summary, target table, counts such as `cursor.rowcount`, verification, and next_action. "
        ...
        "upsert rows into SQLite -> return status, side-effect/count summary, source scope, verification, and next action."
