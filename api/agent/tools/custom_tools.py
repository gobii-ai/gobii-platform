    ...
    return {
        ...
        "function": {
            ...
            "description": (
                ...
                "its return value must make the side effects unambiguous. Keep results concise: status, summary, what changed or which outputs are ready, counts, remaining work/cursor when resumable, and verification guidance are usually enough. "
                ...
                "If the tool already wrote or synced records, make clear that follow-up should be read-only verification, not another append/add/update call. "
...
def get_custom_tools_prompt_summary(agent: PersistentAgent, *, recent_limit: int = 3) -> str:
    ...
    summary = (
        ...
        "For SQLite-backed transforms/syncs, include source/destination table params where appropriate plus date/status filters; for URL/list validators, accept the candidate list and destination/default options as params. "
        "When invoking a custom tool you just created, pass the runtime values you already know; use {} only when the tool intentionally reads verified config/state and returns the resolved targets it used. "
        "Return values must be helpful to the downstream agent, especially after writes or syncs. Keep them concise: status, summary, what changed or which outputs are ready, counts, remaining work/cursor when resumable, and verification guidance are usually enough. "
        ...
        "For validator/classifier tools, return the accepted ready-to-use list, rejected inputs with reasons, the rule used, and whether more inputs are needed. "
        "For completed writes, make clear that the agent should verify with read-only tools instead of manually replaying the same append/add/update calls. "
