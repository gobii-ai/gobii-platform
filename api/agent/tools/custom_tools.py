    ...
    return {
        ...
        "function": {
            ...
            "description": (
                ...
                "Keep DB work inside the `with ctx.sqlite() as db:` block; after the block exits the DB is closed. Use cursor.rowcount/SELECT changes(); call `fetchone()`/`fetchall()` on the cursor returned by `db.execute()`, not on `db`; set db.row_factory = sqlite3.Row before SELECT/fetchall because later changes do not convert tuples and rows are not row.get(...). "
