    ...
    return {
        ...
        "function": {
            ...
            "description": (
                ...
                "per-destination counts, skipped/duplicate counts when relevant, remaining_work/next_cursor for resumable work, verification advice, and the literal key next_action. "
                "Every success or error return dict should include `next_action`, including validation-only/filtering tools. "
...
def get_custom_tools_prompt_summary(agent: PersistentAgent, *, recent_limit: int = 3) -> str:
    ...
    summary = (
        ...
        "For URL/list validators, accept the candidate list or input_table plus output_table and minimum/limit params, then pass concrete values when invoking. "
        ...
        "per-destination rows/items written, skipped/duplicate counts, remaining_work or next_cursor for resumable work, verification advice, and the literal key next_action. "
        "Every success or error return dict should include `next_action`, including validation-only/filtering tools. "
