...
EMAIL_TAGS = tuple("caption div em h1 h2 h3 h4 h5 h6 li ol p span strong table tbody td tfoot th thead tr ul".split())
STYLED_EMAIL_TAGS = tuple(tag for tag in EMAIL_TAGS if tag not in {"br", "img", "pre", "tbody", "tfoot", "thead"})
EMAIL_CSS_PROPERTIES = tuple("background border-bottom border-left border-radius color display flex-direction font-size gap line-height margin margin-bottom margin-top padding padding-bottom".split())
...
def _build_html_cleaner(*, allow_cid: bool = False) -> Cleaner:
    ...
    return Cleaner(tags=sorted(set(ALLOWED_TAGS).union(EMAIL_TAGS, {"br", "img", "pre"})), attributes=attributes, protocols=protocols, css_sanitizer=CSSSanitizer(allowed_css_properties=EMAIL_CSS_PROPERTIES), strip=True)
...
def normalize_explicit_email_html(explicit_html: Any) -> str | None:
    return (explicit_html.strip() or None) if isinstance(explicit_html, str) else None


def sanitize_chat_email_html(html: str | None, *, allow_cid: bool = False) -> str:
    return (HTML_CID_CLEANER if allow_cid else HTML_CLEANER).clean(html) if html else ""


def render_chat_email_body_html(body: str | None, *, explicit_html: str | None = None, rendered_html: str | None = None, allow_cid: bool = False) -> str:
...
def merge_chat_body_html_cache(raw_payload: Mapping[str, Any] | None, body: str | None, *, explicit_html: str | None = None, rendered_html: str | None = None) -> dict[str, Any]:
    payload = dict(raw_payload or {})
    payload[CHAT_BODY_HTML_CACHE_KEY] = render_chat_email_body_html(body, explicit_html=explicit_html, rendered_html=rendered_html, allow_cid=True)
    return payload
