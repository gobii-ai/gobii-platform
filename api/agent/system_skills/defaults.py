...
CONTACTOUT_SYSTEM_SKILL = SystemSkillDefinition(
    ...
    prompt_instructions=(
        ...
        "Only set `years_of_experience` when the user explicitly requests an overall professional-experience range; "
        "skill experience such as Python experience does not imply an overall range. Use `X_Y` strings for experience "
        "ranges, such as `5_30`, only when such a range was requested; never send empty optional arrays or objects. "
        "For `search_people`, use `output_fields` to request only fields needed for the task and keep results compact; "
        "LinkedIn URLs come from `profile_list`, not `output_fields`. Location matching can "
        "include nearby metro areas, so verify returned profile locations against the requested geography and never "
        "silently broaden it. Search results preserve the provider's `content.profiles` map and also include compact "
        "`content.profile_list` entries with `linkedin_url`, `full_name`, `title`, `company_name`, `location`, and "
        "`skills`; prefer `profile_list` for selection and extraction. "
        "Do not combine `match_experience` with `current_titles_only` or `company_filter`; `location_radius` requires "
        "a `location`, and `years_in_current_role` cannot be combined with `recently_changed_jobs=true`. Capitalize "
        "boolean operators such as AND, OR, and NOT where the filter supports them.\n"
        "Contact data is a paid, all-or-none reveal. Keep `reveal_all_contact_info=false` unless the user explicitly "
        "authorizes revealing all available personal emails, work emails, and phone numbers. ContactOut cannot "
        "reveal only one contact category. `required_contact_data_types` is only an availability filter; it does not "
        "limit which fields are returned or billed when contact data is revealed. If the user requests only selected "
        "categories, do not reveal contact data; explain the limitation and ask whether revealing all available "
        "categories is acceptable, or use an approved alternative. A request to find, qualify, summarize, or "
        "shortlist people does not itself authorize revealing contact data.\n"
...
DISCORD_NATIVE_SYSTEM_SKILL = SystemSkillDefinition(
    ...
    prompt_instructions=(
        ...
        "Pass the correct `will_continue_work` value and at least one of `message`, `attachments`, or `embeds`. "
        "Use `embeds` for compact status or summary cards, not routine conversational replies. Simple cards support "
        "`title`, `description`, `url`, `color` as `#RRGGBB`, and `fields` with `name`, `value`, and optional `inline`; "
        "do not invent other embed properties. "
