...
RUNTIME_PLANNING_SYSTEM_SKILL = SystemSkillDefinition(
    ...
    prompt_instructions=(
        "Use `update_plan` to track substantial multi-step work and render useful milestones to the user.\n"
        "Plans are for complex, ambiguous, or multi-phase work; do not use them as filler for simple or single-step queries.\n"
        "Do not create, update, or finish a plan for quick lookups, simple research answers, scheduled briefings, "
        "one-shot chart requests, or simple latest/current company, news, funding, product, status, or batch reports. "
        "For those bounded current-research tasks, gather the small source set and send the final answer directly.\n"
        "For explicit deep or exhaustive research, use at most one initial plan update if the scope benefits from it; "
        "do not call `update_plan` again just to mark research done, narrate progress, attach the final message, or prepare the final response. "
        ...
        "Keep plans short and current: usually 3-6 active steps, with more only when the current task genuinely needs it. "
        ...
        "If the current plan mostly reflects prior completed work, an old scheduled run, or a different task, reset it by replacing those stale steps with a short active plan.\n"
        "For recurring or hourly jobs, do not create one plan step per day, hour, or recurrence slot. Represent only the current run with compact reusable phases such as `Fetch current data`, `Analyze changes`, and `Send report`.\n"
        "Deliverables are top-level `files` and `messages`, not per-step. Use `files` only for final user-visible artifacts; omit scratch files and temporary downloads. "
        "Use `messages` only for final user-facing send_email/send_sms/send_chat_message deliverables with exact returned message_id UUIDs. "
        "If the final message is being sent now and a completion plan update is still genuinely needed, send it first with will_continue_work=true, then call update_plan. "
        "Do not include routine progress updates, peer agent messages from send_agent_message, placeholders, SQL snippets, URLs, or invented IDs.\n"
        "Because deliverables are replaced on every call, preserve only still-relevant previous `files` and `messages` when updating the active plan.\n"
        "After calling `update_plan`, do not repeat the whole plan in chat because the harness displays it.\n"
        "In active planned work outside explicit deep research, mark a previous phase `done` only after verifying it actually succeeded. "
        "If the plan changes mid-task, call `update_plan` again with the full revised plan.\n"
        "For active planned work outside explicit deep research, send the final user-facing report before marking the final step done. "
        "When the task genuinely needs a completion plan update, call `update_plan` with all steps marked `done`.\n\n"
        "Use a plan when the task is non-trivial beyond bounded source gathering/final answer, has phases, "
        "dependencies, ambiguity, multiple requested parts, explicit TODO/plan instructions, or newly discovered "
        "required steps."
    ),
