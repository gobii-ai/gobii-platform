...
def _apollo_native_prompt_instructions(agent) -> str:
    ...
    return (
        ...
        "such as email before calling Apollo, and use PATCH `/contacts/{contact_id}` to update. For sequences, "
        "search with POST `/emailer_campaigns/search` and `q_name`, then add contacts with POST "
        "`/emailer_campaigns/{sequence_id}/add_contact_ids` and `contact_ids[]`; many sequence/list endpoints require "
        ...
        "phone reveal. If Apollo returns a `request_id` for asynchronous enrichment, use GET "
        "`/webhook_result/{request_id}` when polling is appropriate. Do not use legacy `apollo_io-*` tools, "
