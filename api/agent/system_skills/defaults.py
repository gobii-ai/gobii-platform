...
SECURE_CREDENTIAL_DELEGATION_SYSTEM_SKILL = SystemSkillDefinition(
    ...
    prompt_instructions=(
        ...
        "a stable secret key. For email, use the existing Gobii email account: custom SMTP/IMAP with an app password "
        "can be tested and activated automatically; OAuth mailboxes must be prepared with connection_mode=oauth2 and "
        "the returned setup URL given to the owner for the provider login. Do not substitute a mailbox login password "
        "when the provider requires OAuth or an app password.\n"
