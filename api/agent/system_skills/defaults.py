...
RUNTIME_PLANNING_SYSTEM_SKILL = SystemSkillDefinition(
    ...
    prompt_instructions=(
        ...
        "If the current plan mostly reflects prior completed work, an old scheduled run, or a different task, reset it by replacing those stale steps with a short plan for the active task or run.\n"
        ...
        "Use `messages` only after sending a final user-facing report, answer, or summary through send_email, send_sms, or send_chat_message. Use the exact returned message_id UUID, or omit `messages`. If you are sending the final message in this turn and a completion plan update is still genuinely needed, call the send tool first with will_continue_work=true, then call update_plan after the send tool returns. For explicit deep or exhaustive research with no file deliverables, send the final answer with will_continue_work=false and do not call update_plan again just to attach the message or mark steps done. Do not include routine progress updates, peer agent messages from send_agent_message, placeholders, SQL snippets, URLs, or invented IDs.\n"
        ...
        "Because deliverables are replaced on every call, preserve only still-relevant previous `files` and `messages` when later updating the active plan.\n"
        "After calling `update_plan`, do not repeat the whole plan in chat because the harness displays it.\n"
        "Before moving to the next command or phase in an active plan outside explicit deep research, mark the previous step `done` after verifying the work actually succeeded.\n"
        "If the plan changes mid-task in active planned work, call `update_plan` again with the full revised plan.\n"
        ...
        "Use a plan when:\n"
        "- The task is non-trivial and requires multiple actions beyond bounded source gathering and a final answer.\n"
