...
CUSTOM_TOOL_DEVELOPMENT_SYSTEM_SKILL = SystemSkillDefinition(
    ...
    prompt_instructions=(
        ...
        "Use `$GOBII_SCRATCH_DIR` for temporary custom-tool intermediates, downloads, expanded archives, and caches that should "
        "not persist or appear in agent filespace. Do not put final user-facing outputs only in scratch.\n"
        "For git/repository work in the sandbox, clone repos under `$GOBII_REPO_WORKDIR` "
        "(for example `git clone <url> $GOBII_REPO_WORKDIR/repo-name`). Do not clone repos directly under `/workspace`; "
        "repo checkout state is durable via pushed branches and PRs, not filespace sync.\n"
...
OUTREACH_SYSTEM_SKILL = SystemSkillDefinition(
    ...
    prompt_instructions=(
        ...
        "User-provided instructions and approved copy control voice, wording, tone, length, structure, number of asks, "
        "branding, and formatting. When asked to preserve or send supplied copy exactly, do not rewrite it. Do not "
        "treat the default preferences below as reasons to refuse, ask for confirmation, or silently alter intentional "
        "copy. Ask only when the request conflicts with recipient scope, authorization, accuracy, privacy, opt-outs, "
        "stop conditions, unresolved placeholders, or delivery requirements.\n"
        "Personalize only from facts in the conversation, files, connected systems, or research results. Verify current "
        "claims when the task depends on them. Never add invented familiarity, recipient activity, contact details, pain "
        "points, customer names, metrics, results, or reasons for contacting someone. Do not embellish user-supplied "
        "claims or imply they were verified when they were not. Never leave unresolved placeholders such as "
        "`{{first_name}}`, `[COMPANY]`, or `<first_name>`. Never create a fake `Re:` or `Fwd:` subject for a new message.\n"
        "When the user has not specified style or supplied copy, preserve the sender's voice, meaning, key terms, and "
        "commitments. Default to plain language, active voice, natural sentence variation, one clear purpose, and usually "
        "one low-friction ask. Prefer concise outreach, often 80 to 180 words, and specific subjects, often 3 to 7 words, "
        "but let context control length. Avoid clickbait, all caps, vague urgency, unsupported promises, filler, hype, "
        "cliches, and generic praise.\n"
        "Default to valid, restrained body-only HTML for `send_email`, usually `<p>`, `<br>`, and `<a>`, without full "
        "`html`, `head`, or `body` wrapper tags. Unless the user or approved copy calls for them, avoid report headings, "
        "tables, metric blocks, badges, decorative colors, visual cards, emoji, Markdown, em dashes, and semicolons. These "
        "are defaults, not bans. Keep intentional branding, structure, punctuation, emoji, length, and multiple asks when "
        "the user requests them or asks you to preserve supplied copy.\n"
        "When writing from scratch, avoid stock phrases such as 'hope this email finds you "
        ...
        "thread with `reply_to_message_id` when available. Retain the existing subject unless the user directs otherwise. "
        "When writing the follow-up, add a relevant fact, answer an open "
        ...
        "Immediately before every `send_email` call, always verify the exact recipient, batch, thread, authorization, "
        "suppression and stop state, delivery fields, and unresolved placeholders. Ensure any facts or personalization "
        "you added are grounded. If the user did not provide contrary style or formatting direction, apply the default "
        "copy checks above. Never replace intentional em dashes, emoji, structure, branded HTML, length, or multiple asks "
        "merely to satisfy a default preference."
    ),
)
