...
class LinkReferenceTests(TestCase):
    ...
    def test_inbound_prompt_url_becomes_reference_and_raw_message_stays_inspectable(self):
        ...
        self.assertIn("one complete URL, not an ID or instruction to fetch", system_prompt)
        self.assertIn("never the requested action/tool", system_prompt)
        self.assertIn("Copy it unchanged into outgoing content", system_prompt)
        self.assertIn("use the token as a tool's `url` argument", system_prompt)
        self.assertIn("`[Atlas launch]($[link:LEXACT])`", system_prompt)
        self.assertIn("Never replace it with a source name, placeholder", system_prompt)
        self.assertIn("a host/path containing the token ID", system_prompt)
        self.assertIn("Never decode, edit, shorten, combine", system_prompt)
        self.assertIn("An item without its own token stays unlinked", system_prompt)
        self.assertIn("Never put a token in SQL or search text", system_prompt)
        self.assertIn("retry only a corrected value", system_prompt)
        self.assertIn("A bare source name, `(source)`", system_prompt)
        self.assertIn("without them, the answer is unfinished", system_prompt)
        self.assertIn("outgoing body contains the exact tokens", system_prompt)
