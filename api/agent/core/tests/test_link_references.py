...
class LinkReferenceTests(TestCase):
    ...
    def test_inbound_prompt_url_becomes_reference_and_raw_message_stays_inspectable(self):
        ...
        self.assertIn("the token itself is the URL, not an ID or suffix", system_prompt)
        self.assertIn("safe and required to copy the token unchanged into outgoing content", system_prompt)
        self.assertIn("exact whole token as a tool's `url` argument", system_prompt)
