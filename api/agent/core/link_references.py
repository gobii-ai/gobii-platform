...
_NAKED_DESTINATION_RE = re.compile(
    rf"(?:\]\(\s*|href\s*=\s*['\"]\s*)({_PUBLIC_ID_PATTERN})(?=\s*(?:\)|['\"]))", re.IGNORECASE
)
        ...
        PersistentAgentLinkReference.objects.bulk_create(
            [
                PersistentAgentLinkReference(
                    agent=agent, url=url, url_hash=url_hash, source_kind=source_kind,
...
def rewrite_prompt_urls(
    ...
    try:
        references = _reference_map(
            agent, urls, create=create, source_kind=source_kind, source_object_id=source_object_id
        )
...
def register_prompt_urls(text: str, agent, *, source_kind: str, source_object_id: str = "") -> None:
    try:
        _reference_map(
            agent, extract_http_urls(text), create=True, source_kind=source_kind, source_object_id=source_object_id
        )
    except DatabaseError:
...
def _resolution_error(message: str) -> LinkReferenceResolutionError:
    return LinkReferenceResolutionError(message)
...
def resolve_link_references(text: str, agent) -> str:
    ...
    if naked := _NAKED_DESTINATION_RE.search(text):
        ...
        raise _resolution_error(f"A link reference is malformed. Use $[link:{public_id}] as the complete destination.")
    matches = list(_REFERENCE_RE.finditer(text))
    if len(matches) != len(_REFERENCE_PREFIX_RE.findall(text)):
        raise _resolution_error("A link reference is malformed. Reuse a provided $[link:id] value or omit the link.")
    rendered = {
        url: public_id
    ...
    try:
        references = {
            ...
            for reference in PersistentAgentLinkReference.objects.filter(agent=agent, public_id__in=lookup_ids)
        }
    except DatabaseError as exc:
        raise _resolution_error("Link references are temporarily unavailable. Retry the same message.") from exc
    if not lookup_ids.issubset(references):
        missing = ", ".join(sorted(lookup_ids - references.keys()))
        raise _resolution_error(f"Link reference {missing} is unavailable. Recopy that provided token exactly; other references remain usable.")
...
def resolve_link_reference_params(value, agent):
    ...
    if not (_REFERENCE_RE.fullmatch(stripped) or _rendered_reference_id(stripped)):
        return value
