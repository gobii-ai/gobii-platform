...
from .credit_gating import (
    CreditSnapshot,
    prepare_processing_credit_state,
    reserve_tool_credit,
)
...
def _get_batch_daily_credit_state(
    ...
    daily_state = credit_snapshot.get("daily_state") if isinstance(credit_snapshot, CreditSnapshot) else None
    if daily_state is None and isinstance(credit_snapshot, dict):
    ...
    try:
        ...
        daily_state = None
    if isinstance(credit_snapshot, (dict, CreditSnapshot)):
        ...
        with tracer.start_as_current_span("Prepare Tool") as tool_span:
            ...
            if tool_name == "sleep_until_next_trigger":
                if has_non_sleep_calls:
                    ...
                    continue
                credit_decision = reserve_tool_credit(
                    agent,
                    tool_name,
                    span=tool_span,
...
def _process_agent_events_locked(
    ...
    *,
    ...
    try:
        ...
        credit_state = prepare_processing_credit_state(
            agent,
            budget_ctx=budget_ctx,
            span=span,
        )
        credit_snapshot = credit_state.credit_snapshot
        if not credit_state.continue_processing:
            return agent
