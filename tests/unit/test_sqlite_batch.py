...
class SqliteBatchCoreTests(SqliteBatchTestCase):
    ...
    def test_tool_definition_guides_domain_modeling_and_bulk_work(self):
        ...
        for expected in (
            "MESSAGE RULE",
            "state/status must use state=:source_status or json_extract",
            "never a literal",
            "bind/derive every payload field from evidence",
            ...
            "no ID/URL filters",
            "Peer/message fields never use rows/__tool_results",
            "from __messages",
            "bound evidence",
            "every mutable/provenance field and durable key",
            "no sourced SQL literal",
