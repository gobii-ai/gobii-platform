...
class MarketingEventsApiTests(SimpleTestCase):
    @override_settings(GOBII_PROPRIETARY_MODE=True, CAPI_COMPLETE_REGISTRATION_DELAY_MINUTES=60)
    @patch("marketing_events.api.time.time", return_value=1_700_000_000)
    @patch("marketing_events.api.enqueue_complete_registration_marketing_event.apply_async")
    def test_capi_complete_registration_delays_with_original_event_time(
        self,
        mock_apply_async,
        _mock_time,
    ):
        ...
        self.assertEqual(payload["event_name"], "CompleteRegistration")
        self.assertEqual(payload["properties"]["event_id"], "evt-123")
    ...
    @override_settings(
        ...
        CAPI_COMPLETE_REGISTRATION_DELAY_MINUTES=60,
        CAPI_START_TRIAL_DELAY_MINUTES=60,
    )
    @patch("marketing_events.api.enqueue_marketing_event.delay")
    @patch("marketing_events.api.enqueue_complete_registration_marketing_event.apply_async")
    @patch("marketing_events.api.enqueue_start_trial_marketing_event.apply_async")
    def test_capi_non_delayed_event_uses_existing_enqueue_path(
        self,
        mock_start_trial_apply_async,
        mock_complete_registration_apply_async,
        mock_enqueue_delay,
    ):
        user = SimpleNamespace(id=42, email="test@example.com", phone="+15555550123")
        ...
        mock_complete_registration_apply_async.assert_not_called()
