...
class TestLLMFailover(TestCase):
    ...
    def test_azure_non_openai_persistent_endpoint_stays_on_completion_api(self):
        clear_llm_db()
        LLMProvider = apps.get_model('api', 'LLMProvider')
        PersistentModelEndpoint = apps.get_model('api', 'PersistentModelEndpoint')
        PersistentTokenRange = apps.get_model('api', 'PersistentTokenRange')
        ...
        self.assertEqual(params.get("api_base"), 'https://example.services.ai.azure.com')
        self.assertEqual(params.get("api_version"), 'v1')
        self.assertEqual(params.get("api_key"), 'azure-key')
        self.assertEqual(params.get("custom_llm_provider"), 'azure')
        self.assertEqual(params.get("reasoning_effort"), 'medium')
    ...
    @override_settings(GOBII_PROPRIETARY_MODE=True)
    def test_new_free_account_uses_standard_tier(self):
        clear_llm_db()
        seeded = self._seed_premium_setup(include_premium=True)
    ...
    @override_settings(GOBII_PROPRIETARY_MODE=True)
    def test_free_user_first_loop_uses_standard_tier(self):
        clear_llm_db()
        seeded = self._seed_premium_setup(include_premium=True)
        agent = self._make_agent_stub(plan_id="free", days_since_joined=90)
    ...
    @override_settings(GOBII_PROPRIETARY_MODE=True)
    def test_paid_user_first_loop_keeps_standard_preference(self):
        clear_llm_db()
        seeded = self._seed_premium_setup(include_premium=True)
        agent = self._make_agent_stub(plan_id="startup", days_since_joined=90)
        agent.preferred_llm_tier = get_intelligence_tier("standard")

        with mock.patch.dict(os.environ, {"ANTHROPIC_API_KEY": "sk-paid"}, clear=True):
            configs = get_llm_config_with_failover(
                token_count=0,
                agent=agent,
                agent_id=str(agent.id),
    ...
    def test_agent_judge_uses_dedicated_endpoint_then_highest_regular_tier(self):
        ...
        judge_endpoint = PersistentModelEndpoint.objects.create(
            key='judge_model',
            provider=provider,
            enabled=True,
            litellm_model='openrouter/openai/gpt-5.1',
            supports_tool_choice=True,
        )
        fallback_endpoint = PersistentModelEndpoint.objects.create(
            key='fallback_default',
        ...
        profile = LLMRoutingProfile.objects.create(
            ...
            is_active=False,
            agent_judge_endpoint=judge_endpoint,
        )
        token_range = ProfileTokenRange.objects.create(profile=profile, name='default', min_tokens=0, max_tokens=None)
        tier = ProfilePersistentTier.objects.create(
            token_range=token_range,
        ...
        self.assertEqual(provider_key, judge_endpoint.key)
        self.assertEqual(model, judge_endpoint.litellm_model)
