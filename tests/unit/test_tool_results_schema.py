...
1,Alice"""
    ...
    def test_scrape_as_markdown_preview_uses_plain_markdown_and_meta_guidance(self):
        ...
        self.assertIn("Next sqlite_batch call", prompt_info.meta)
        self.assertIn("top-level `rows`", prompt_info.meta)
        self.assertIn("FROM json_each(:rows)", prompt_info.meta)
        self.assertIn("stores result_id plus each exact known URL", prompt_info.meta)
        self.assertIn("Source facts belong in bound rows, not SQL text", prompt_info.meta)
        self.assertIn("Never json_each(result_text)", prompt_info.meta)
        self.assertIn("inspect analysis_json/result_json", prompt_info.meta)
        self.assertIn("read_file", prompt_info.meta)
    ...
    def test_scrape_as_markdown_meta_does_not_misclassify_comma_heavy_page_as_csv(self):
        ...
        self.assertIn("SCRAPE MARKDOWN", prompt_info.meta)
        self.assertIn("result_text is plain page text", prompt_info.meta)
