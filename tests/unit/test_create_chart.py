...
class LabelLenTests(TestCase):
    ...
    def test_mixed_types_converted_to_str(self):
        # Numbers are converted via str() before measuring
...
class ShouldSwapToHorizontalTests(TestCase):
    ...
    def test_non_bar_type_never_swaps(self):
        long_labels = ["A Very Long Label Name"] * 3
    ...
    def test_bar_with_short_labels_does_not_swap(self):
        # Labels ≤ threshold should not trigger swap
        short_labels = ["Jan", "Feb", "Mar"]
        self.assertFalse(_should_swap_to_horizontal("bar", short_labels))

    def test_bar_with_labels_exactly_at_threshold_does_not_swap(self):
        # Labels at or below threshold do not trigger swap; only labels strictly longer do.
        labels = ["x" * _LONG_LABEL_THRESHOLD]
        self.assertFalse(_should_swap_to_horizontal("bar", labels))

    def test_bar_with_labels_one_over_threshold_swaps(self):
        labels = ["x" * (_LONG_LABEL_THRESHOLD + 1)]
    ...
    def test_bar_with_numeric_labels_below_threshold(self):
        # Short numeric labels (converted to str) do not trigger swap
        self.assertFalse(_should_swap_to_horizontal("bar", [1, 2, 3]))

    def test_bar_with_large_numeric_labels_exceeding_threshold_swaps(self):
        # Large integers whose str() representation exceeds the threshold trigger swap
        labels = [1234567890123]  # 13 chars when str()-converted
        self.assertTrue(_should_swap_to_horizontal("bar", labels))
...
class AxisSwapIntegrationTests(TestCase):
    ...
    def setUpTestData(cls):
        User = get_user_model()
        cls.user = User.objects.create_user(
            username="axisswap@example.com",
            email="axisswap@example.com",
            password="secret",
        )
        cls.browser_agent = BrowserUseAgent.objects.create(user=cls.user, name="Swap Browser")
        cls.agent = PersistentAgent.objects.create(
            user=cls.user,
            name="Swap Agent",
            charter="test axis swapping",
            browser_use_agent=cls.browser_agent,
        )
    ...
    def _make_svg(self, chart_type, data, x="cat", y="val", **kwargs):
        """Helper: call _generate_chart and return decoded SVG string."""
    ...
    def test_bar_with_short_labels_renders_vertical_bars(self):
        """Short labels → vertical bar chart; x-axis should have category tick labels."""
        svg = self._make_svg("bar", self.SHORT_DATA)
        self.assertIn("<svg", svg)
        # Vertical bars render rect elements
        self.assertIn("rect", svg)

    def test_bar_with_long_labels_renders_horizontal_bars(self):
        """Long labels → auto-swapped to horizontal bar; SVG should still render."""
        svg = self._make_svg("bar", self.LONG_LABEL_DATA)
        self.assertIn("<svg", svg)
        self.assertIn("rect", svg)

    def test_bar_with_long_labels_svg_contains_label_text(self):
        """Long labels must appear in the SVG output (not truncated/absent)."""
        svg = self._make_svg("bar", self.LONG_LABEL_DATA)
        self.assertIn("North America", svg)

    def test_explicit_horizontal_bar_with_long_labels_svg_contains_label_text(self):
        """Explicit horizontal_bar with long labels must appear in SVG output."""
    ...
    @patch("api.agent.files.filespace_service.write_bytes_to_dir")
    @patch("api.agent.files.attachment_helpers.build_signed_filespace_download_url")
    def test_execute_create_chart_swaps_bar_to_horizontal_for_long_labels(
        self, mock_signed_url, mock_write
    ):
