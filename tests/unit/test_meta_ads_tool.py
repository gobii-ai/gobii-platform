...
from api.models import BrowserUseAgent, GlobalSecret, PersistentAgent, PersistentAgentEnabledTool
...
class MetaAdsToolTests(TestCase):
    ...
    def test_execute_meta_ads_requires_native_integration_setup_when_missing(self):
        result = execute_meta_ads(self.agent, {"operation": "doctor"})

        self.assertEqual(result["status"], "action_required")
        self.assertEqual(result["skill_key"], "meta_ads_platform")
        self.assertEqual(result["provider_key"], "meta_ads")
        self.assertIn("META_APP_ID", result["required_fields"])
        self.assertIn("/app/integrations", result["setup_url"])
        self.assertIn("provider=meta_ads", result["setup_url"])
        self.assertIn("connect=1", result["setup_url"])
        self.assertGreater(len(result["setup_steps"]), 0)
        self.assertGreater(len(result["setup_docs"]), 0)

    def test_execute_meta_ads_rejects_legacy_profile_key(self):
        _create_meta_integration(user=self.user)

        result = execute_meta_ads(self.agent, {"operation": "doctor", "profile_key": "default"})

        self.assertEqual(result["status"], "error")
        self.assertEqual(result["code"], "meta_ads_profiles_removed")
        self.assertIn("profiles were replaced", result["message"])
