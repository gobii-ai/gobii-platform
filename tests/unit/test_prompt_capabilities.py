...
class AgentCapabilitiesPromptTests(TestCase):
    ...
    def test_sandbox_summary_biases_toward_custom_tools_for_bulk_work(self, _mock_sandbox):
        ...
        self.assertIn("Default for repetitive, paginated, bulk, or deterministic data work is a small custom tool", summary)
        self.assertIn("especially MCP/API fan-out", summary)
        self.assertIn("bulk SQLite writes", summary)
        self.assertIn("source_path='/tools/name.py'", summary)
        self.assertIn("source_code", summary)
        self.assertIn("retry create_custom_tool, not create_file", summary)
