...
class PromptContextSqlitePlacementTests(TestCase):
    ...
    def test_sqlite_guidance_only_in_system_message(self):
        ...
        self.assertIn("</sqlite_guidance>", system_message["content"])
        self.assertIn("Named tables are the durable world model and exact logic layer", sqlite_guidance)
        self.assertIn("Query truth instead of remembering it", sqlite_guidance)
