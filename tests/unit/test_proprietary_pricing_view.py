...
class PricingPageCtaCopyTests(TestCase):
    ...
    @override_settings(GOBII_PROPRIETARY_MODE=True)
    @patch("proprietary.views.get_active_public_plan_context", return_value=None)
    @patch("proprietary.views.get_stripe_settings")
    def test_team_offer_uses_static_defaults_when_public_plan_context_is_missing(
        self,
        mock_get_stripe_settings,
        _mock_get_active_public_plan_context,
    ):
        mock_get_stripe_settings.return_value = SimpleNamespace(
            startup_trial_days=7,
            scale_trial_days=14,
        ...
        teams_response = self.client.get(reverse("proprietary:teams"))
        pricing_response = self.client.get(reverse("proprietary:pricing"))

        for response in (teams_response, pricing_response):
            self.assertEqual(response.status_code, 200)
            self.assertContains(response, "$50")
    ...
    @override_settings(GOBII_PROPRIETARY_MODE=True)
    @patch("proprietary.views.evaluate_user_trial_eligibility", return_value=SimpleNamespace(decision="eligible"))
    @patch("proprietary.views.user_has_prior_individual_history", return_value=True)
    @patch("proprietary.views.get_user_plan", return_value={"id": PlanNames.FREE})
    @patch("proprietary.views.get_stripe_settings")
    def test_free_user_pricing_hides_trial_copy_when_one_per_user_flag_blocks_user(
        self,
        mock_get_stripe_settings,
        _mock_get_user_plan,
        mock_prior_history,
        ...
        user = get_user_model().objects.create_user(
            username="pricing-one-per-user@example.com",
            email="pricing-one-per-user@example.com",
            password="pw",
        )
        self.client.force_login(user)
