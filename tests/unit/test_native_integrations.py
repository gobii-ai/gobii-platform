...
class NativeIntegrationTests(TestCase):
    ...
    def test_google_sheets_prompt_tells_agent_how_to_discover_accessible_spreadsheets(self):
        ...
        self.assertIn("answer with the accessible matching spreadsheet names and IDs/links", block)
        self.assertIn("Do not ask a clarification question just because a returned match is not an exact title match", block)
        self.assertIn("When the user names an A1 range", block)
        self.assertIn("/v4/spreadsheets/{spreadsheetId}/values/{url_encoded_range}", block)
        self.assertIn("correct the documented endpoint once", block)
    ...
    def test_hubspot_prompt_tells_agent_how_to_use_native_rest_api(self):
        ...
        self.assertIn("summarize the returned JSON content directly", block)
        self.assertIn("Do not call `read_file` for `$[/object/result_*]` placeholders", block)
