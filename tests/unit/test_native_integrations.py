...
class NativeIntegrationTests(TestCase):
    ...
    def test_google_sheets_prompt_tells_agent_how_to_discover_accessible_spreadsheets(self):
        ...
        self.assertIn("your first call should be a Sheets endpoint", block)
        self.assertIn("List accessible spreadsheets", block)
        self.assertIn("only when the user gives a sheet title/name instead of an ID", block)
        self.assertIn("Do not call Drive with partial filters", block)
        self.assertIn("never in `headers` or `headers.params`", block)
        self.assertIn("percent-encode quotes in `q` as `%27`", block)
        self.assertIn("Do not use web search, `search_tools`, or public `docs.google.com` results", block)
        ...
        self.assertIn("complete base query rather than a partial predicate", block)
        self.assertIn("Put query params in the URL, not headers", block)
        self.assertIn("encode quotes as `%27`", block)
        ...
        self.assertIn("Do not use Drive discovery for a known spreadsheet ID", block)
