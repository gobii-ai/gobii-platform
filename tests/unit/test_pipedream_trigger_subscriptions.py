...
from api.services.pipedream_trigger_subscriptions import DISCORD_MESSAGE_EVENT_TYPE
...
class ConnectedAppChannelsSystemSkillTests(TestCase):
    ...
    def test_discord_native_skill_does_not_request_discord_ids_as_secrets(self):
        ...
        self.assertIn("call `ensure` with the selected `guild_id` and `channel_name`", instructions)
        self.assertIn("Never ask for or pass a Discord channel ID", instructions)
