...
class PromptContextSqliteGuidanceTests(SimpleTestCase):
    ...
    def test_sqlite_guidance_tracks_bounded_set_coverage(self):
        ...
        self.assertIn("Fresh peer outcomes supersede stale state", guidance)
        self.assertIn("upsert and read back existing canonical rows from __messages", guidance)
        self.assertIn("recipient/provider/item", guidance)
        self.assertIn("never peer-local keys", guidance)
        self.assertIn("Bind inspected message fields as :params", guidance)
        self.assertIn("never SQL literals/call IDs as result IDs", guidance)
