...
class PromptContextSqliteGuidanceTests(SimpleTestCase):
    def test_sqlite_guidance_tracks_bounded_set_coverage(self):
        ...
        self.assertIn("query them, not memory", guidance)
        self.assertIn("don't refetch complete rows", guidance)
        self.assertIn("For requested fresh data, fetch once", guidance)
        self.assertIn("otherwise query first", guidance)
        self.assertIn("Tool output doesn't update the model", guidance)
        self.assertIn("import each useful batch directly", guidance)
        self.assertIn("checklist, intermediate, and resume state", guidance)
        self.assertIn("stable-ID entities, children/people, relationships", guidance)
        self.assertIn("Reconcile and SELECT in one batch", guidance)
        self.assertIn("evolve schema", guidance)
        self.assertIn("Upserts refresh mutable/provenance fields", guidance)
        self.assertIn("Only unrelated one-offs bypass", guidance)
        self.assertIn("Query wrong counts/gaps", guidance)
        self.assertIn("only sourced blockers are unresolved", guidance)
        self.assertIn("use exact SQL for sets/counts/ranking", guidance)
        self.assertIn("Bind agent-authored values with :name", guidance)
        self.assertIn("No sibling-by-sibling loops", guidance)
