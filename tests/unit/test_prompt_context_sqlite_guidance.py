...
class PromptContextSqliteGuidanceTests(SimpleTestCase):
    def test_sqlite_guidance_tracks_bounded_set_coverage(self):
        ...
        self.assertIn("FIRST SHOT: in one sqlite_batch", guidance)
        self.assertIn("Key entities, children, relations", guidance)
        self.assertIn("decision/evidence SELECTs", guidance)
        self.assertIn("supporting rows and URLs", guidance)
        ...
        self.assertIn("t.is_current_batch=1 AND t.tool_name=:tool", guidance)
        self.assertIn("Never put tool-returned facts in VALUES", guidance)
        self.assertIn("never loop over result_id", guidance)
        self.assertIn("Same-batch, same-shaped tool results are one set", guidance)
        self.assertIn("item fields, including item URLs, from item.value", guidance)
        self.assertIn("only actual parent fields from t.result_json and provenance from t.result_id", guidance)
        ...
        self.assertIn("sqlite_batch's top-level rows argument", guidance)
        self.assertIn("only exact known source URLs", guidance)
        self.assertIn("storing result_id or source_url as row provenance", guidance)
        self.assertIn("never loop over result_id or include every historical call", guidance)
        self.assertIn("SCHEMA FIRST: for an existing table", guidance)
        self.assertIn("listing every table is a failure", guidance)
        self.assertIn("Never combine PRAGMA/schema inspection", guidance)
        self.assertIn("repeat an unchanged SELECT", guidance)
        self.assertIn("use at most two calls", guidance)
        self.assertIn("separate metadata-only call", guidance)
        self.assertIn("query using confirmed tables, columns, and join keys", guidance)
        self.assertIn("After a schema error inspect, don't guess again", guidance)
        self.assertIn("roll back or reconnect", guidance)
