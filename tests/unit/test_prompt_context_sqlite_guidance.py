...
class PromptContextSqliteGuidanceTests(SimpleTestCase):
    ...
    def test_sqlite_guidance_tracks_bounded_set_coverage(self):
        ...
        self.assertIn("Use json_each only on arrays/objects", guidance)
        self.assertIn("json_extract on scalars", guidance)
        self.assertIn("Pass `rows=[]`", guidance)
        self.assertIn("add no source_url/result_id filter", guidance)
        self.assertIn("store t.source_url/t.result_id", guidance)
        ...
        self.assertIn("one top-level interpretation row per result_id", guidance)
        self.assertIn("join it to __tool_results for provenance", guidance)
