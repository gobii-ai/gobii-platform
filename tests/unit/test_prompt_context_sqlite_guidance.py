...
class PromptContextSqliteGuidanceTests(SimpleTestCase):
    ...
    def test_sqlite_guidance_tracks_bounded_set_coverage(self):
        ...
        self.assertIn("`will_continue_work=true` for any read that may trigger another tool", guidance)
        self.assertIn("queue reads are true", guidance)
        self.assertIn("Otherwise false", guidance)
        self.assertIn("Deliver returned rows without validation/reread", guidance)
