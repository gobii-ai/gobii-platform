...
class PromptContextSqliteGuidanceTests(SimpleTestCase):
    def test_sqlite_guidance_tracks_bounded_set_coverage(self):
        ...
        self.assertIn("SELECT decisions", guidance)
        self.assertIn("evidence, and raw URLs", guidance)
        ...
        self.assertIn("t.tool_name=:tool", guidance)
        self.assertIn("Source facts never use VALUES", guidance)
        self.assertIn("per-result_id loops", guidance)
        self.assertIn("Same-shaped tool results are one set", guidance)
        self.assertIn("derive URL/result_id", guidance)
        self.assertIn("don't store link tokens", guidance)
        self.assertIn("Unstructured work binds JSON :rows", guidance)
        self.assertIn("joined by result_id", guidance)
        self.assertIn("SCHEMA FIRST: if an existing definition", guidance)
        self.assertIn("never list every table", guidance)
        self.assertIn("Never combine schema inspection", guidance)
        self.assertIn("repeat an unchanged SELECT", guidance)
        self.assertIn("never a third", guidance)
        self.assertIn("Inspect metadata alone", guidance)
        self.assertIn("query confirmed fields and joins", guidance)
        self.assertIn("After a schema error inspect, don't guess", guidance)
        self.assertIn("roll back/reconnect first", guidance)
