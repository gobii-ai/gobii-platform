...
class PersistentAgentPlanningModeTests(TestCase):
    ...
    def test_first_run_prompt_asks_only_when_role_boundaries_materially_change_work(self):
        ...
        self.assertIn("ask the material decision questions together", prompt)
        self.assertIn("Use the current inbound channel", prompt)
        self.assertIn("one native request_human_input call", prompt)
        self.assertIn("never a numeric quota or a preference survey", prompt)
        self.assertIn("usually 2-3 and never more than 6", prompt)
        self.assertIn("mandatory for this intake even when orientation cannot identify the entity", prompt)
        self.assertIn("plausible interpretations or concrete next paths", prompt)
        self.assertIn("They stay pending if the user leaves", prompt)
        self.assertIn("follow the result guidance", prompt)
        self.assertIn("mirror every question and its exact choices there", prompt)
        self.assertIn("send the same numbered questions and choices there", prompt)
