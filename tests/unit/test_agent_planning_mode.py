...
class PersistentAgentPlanningModeTests(TestCase):
    ...
    def test_normal_prompt_distinguishes_first_assignment_discovery_from_clear_work(self):
        ...
        self.assertIn("Send later evidence updates only when user/Charter cadence permits", prompt)
        self.assertIn("retain findings for the next allowed report", prompt)
        ...
        self.assertIn("each placeholder vs fields", prompt)
        self.assertIn("retryable=false ends that field", prompt)
